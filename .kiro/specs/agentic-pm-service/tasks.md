...
- [x] ~~10. [7순위] 과제 마켓플레이스~~ — 제외 (스코프 축소)

- [x] 11. Checkpoint - 확장 기능 통합 확인
  - Ensure all tests pass, ask the user if questions arise.

- [-] 12. [8순위] 포인트 시스템 (정산 + 실시간 예측 + Best Collaborator)
  - [x] 12.1 포인트 시스템 TypeScript 데이터 모델 추가 (`src/types/index.ts`)
  ...
  - [x] 12.2 포인트 시스템 Reducer 액션 구현 (`src/context/teamReducer.ts`)
  ...
  - [x] 12.3 usePoints 커스텀 훅 구현 (`src/hooks/usePoints.ts`)
  ...
  - [x] 12.5 POST /api/points/settle Lambda 함수 구현
  ...
  - [x] 12.6 POST /api/points/predict Lambda 함수 구현
  ...
  - [x] 12.7 포인트 위젯 UI 구현 (`src/components/points/PointWidget.tsx`)
    - 대시보드 상단에 현재 포인트 잔액, 보증금, 최근 변동 내역 표시
    - 각 팀원 카드에 AI 포인트 예측 표시
    - _Requirements: 23.4_

  - [x] 12.8 프로젝트 종료 정산 화면 구현 (`src/components/points/SettlementScreen.tsx`)
  ...
  - [x] 12.9 포인트 교환 페이지 구현 (`src/components/points/PointExchangePage.tsx`)
